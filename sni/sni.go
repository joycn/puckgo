...
func ReadServerName(r *bufio.Reader) (string, error) {
	...
	case recordTypeHandshake:
		...
		if major != 3 || minor > 3 {
			return "", fmt.Errorf("inval format")
		}
		...
		if msgLen < 4 {
			return "", fmt.Errorf("msg length invalid")
		}
		...
		if len(data) < handshakeLen+4 {
			return "", fmt.Errorf("handshakeLen invalid")
		}

		switch data[0] {
		case typeClientHello:
			serverName, _ := ServerName(data)
