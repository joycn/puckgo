...
func start(cfg *config.Config) error {
	...
	defaultServerAddr, err := net.ResolveUDPAddr("udp", cfg.DefaultServer)
	...
	exceptiveServerAddr, err := net.ResolveUDPAddr("udp", cfg.ExceptiveServer)
	...
	for {
		...
		if len(msg.Question) > 0 {
			...
			if err != nil {
				need = datasource.MatchAction(cfg.DefaultAction)
			}
			m.Lock()
