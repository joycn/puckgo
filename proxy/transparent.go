...
// StartTransparentProxy start proxy to handle http and https
func StartTransparentProxy(ma *datasource.AccessList, tranparentProxyConfig *config.TransparentProxyConfig) {
	...
	if tranparentProxyConfig.SecurityUpstream {
		proxyDialer, _ = proxy.SOCKS5("tcp", tranparentProxyConfig.ProxyUpstream, nil, conn.TLSDialer)
	} else {
		proxyDialer, _ = proxy.SOCKS5("tcp", tranparentProxyConfig.ProxyUpstream, nil, proxy.Direct)
	}
